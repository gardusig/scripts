...
UNIT_TEST_INSTRUCTION = Instruction(
    instructions=[
        ...
        "Use the pytest framework; place all tests under a top-level `tests/` directory.",
        "Name each test file following the pattern:\n"
        "  `src/path/to/module.py` → `tests/path/to/test_module.py`.",
        "🔧 When you need to replace behavior or defaults in another module or class,",
        ...
        "  • Example: `monkeypatch.setattr('kirby.ai.aws.bedrock_config.BedrockClientConfig', 'max_tokens', 4096)`",
        "Stub every external dependency (I/O, network, DB) by patching the symbol in the module under test:",
        "  • Don’t patch the import path of the library; patch the name as it appears in your module.",
        "    E.g. `patch('myapp.module.external_api', fake_api)` not `patch('external_lib.api', fake_api)`.",
        "Use pytest fixtures (`tmp_path`, `monkeypatch`) for setup/teardown and filesystem isolation.",
        "Leverage `pytest.mark.parametrize` to cover multiple input/output cases in one function.",
        ...
        "Give each test function a clear, descriptive `snake_case` name stating the behavior under test.",
