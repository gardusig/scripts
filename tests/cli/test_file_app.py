...
def mock_dependencies():
    with (
        patch("prompt_craft.cli.file_app.append_shared_file") as mock_append,
        patch("prompt_craft.cli.file_app.remove_shared_file") as mock_remove,
        patch("prompt_craft.cli.file_app.clear_shared_files") as mock_clear,
        patch(
            "prompt_craft.cli.file_app.summary_shared_files",
            return_value="Summary of shared files",
        ) as mock_summary,
        patch("prompt_craft.cli.file_app.undo_shared_files") as mock_undo,
    ):
        yield {
            "append": mock_append,
            "remove": mock_remove,
...
def test_add_file(mock_dependencies):
    with patch(
        "prompt_craft.cli.file_app.get_all_files",
