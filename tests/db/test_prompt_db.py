...
patch_target_create_session_file = "prompt_craft.db.prompt_db.create_session_file"
patch_target_HistoryDB = "prompt_craft.db.prompt_db.HistoryDB"
...
def patch_external_deps(monkeypatch):
    ...
    monkeypatch.setattr(
        "prompt_craft.db.prompt_db.create_session_file", lambda name: f"/tmp/{name}.db"
    )
    # Patch HistoryDB with a MagicMock
    mock_history_db_cls = MagicMock()
    monkeypatch.setattr("prompt_craft.db.prompt_db.HistoryDB", mock_history_db_cls)
    yield
...
def prompt_store(monkeypatch):
    ...
    from prompt_craft.db import prompt_db
