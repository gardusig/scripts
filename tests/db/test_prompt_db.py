...
patch_target_create_session_file = "kirby.db.prompt_db.create_session_file"
patch_target_HistoryDB = "kirby.db.prompt_db.HistoryDB"
...
def patch_external_deps(monkeypatch):
    ...
    monkeypatch.setattr(
        "kirby.db.prompt_db.create_session_file", lambda name: f"/tmp/{name}.db"
    )
    # Patch HistoryDB with a MagicMock
    mock_history_db_cls = MagicMock()
    monkeypatch.setattr("kirby.db.prompt_db.HistoryDB", mock_history_db_cls)
    yield
...
def prompt_store(monkeypatch):
    ...
    from kirby.db import prompt_db
